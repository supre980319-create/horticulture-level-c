...
(function(){
  ...
  function pickWrongOptions(allItems, correctItem){
    // 規則：
    // 1) 錯誤選項必須來自「同一個資料夾」(folder 相同)
    // 2) 但中文名稱(name) 必須跟正確答案不同（避免同名不同角度）
    const correctName = getChineseName(correctItem);
    const correctFolder = (correctItem.folder || "").trim();

    // 先用「同資料夾」當候選
    let candidates = allItems.filter(it =>
      (it.folder || "").trim() === correctFolder &&
      getChineseName(it) &&
      getChineseName(it) !== correctName
    );

    // 如果同資料夾找不到足夠 3 個，就退一步：同 category
    if(candidates.length < 3){
      const cat = (correctItem.category || "").trim();
      candidates = allItems.filter(it =>
        (it.category || "").trim() === cat &&
        getChineseName(it) &&
        getChineseName(it) !== correctName
      );
    }
    ...
    for(const it of shuffle(candidates)){
      ...
      if(!nm || usedNames.has(nm)) continue;
      usedNames.add(nm);
      picked.push(it);
      if(picked.length === 3) break;
    }
    if(picked.length < 3){
      throw new Error("Wrong options not enough (need 3 different Chinese names in same folder/category)");
    }
    return picked;
  }
